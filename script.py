...
for i in list:
    im = Image.open('Certificate.png') # certificate image
    imgh= im.height
    w=(im.width)
    d = ImageDraw.Draw(im)
    font = ImageFont.truetype("SF-Pro-Display-Medium.otf", 52) # font file and size

    t= d.textsize(i,font=font)[0]
    location = (
        ((w - t)/2)-292, 670) # location of name
    text_color = (0, 0, 0)
    ...
    for n in list1:
        ...
        location = (
            ((w - t) / 2) - 292, 834) # location of event 
        text_color = (0, 0, 0)
